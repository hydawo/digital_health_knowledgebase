...
| ~~113~~ | **LifeData now has a Module 2 profile**, so Nock et al. 2026 (`10.1037/abn0001117`, N=619) is no longer excluded on scope grounds. Profiled 2026-09-03 as `lifedata-post-hospital-suicide-ema` | | Closed |
...
| 123 | **Three Module 4 boundary calls from the 2026-09-03 baseline search.** (a) Hassan et al. 2025, *European Psychiatry*, recommendations for consumer wearables in serious mental illness, has no DOI or abstract in Europe PMC and looks like a congress abstract; is there a full paper? (b) Three validation-framework papers were left out to hold the baseline near 40 and sit on the Module 1 and Module 4 boundary: INTERLIVE energy-expenditure statement (PMC9325806), Menghini's sleep-staging validation framework (PMC8161815), Welk's activity-monitor reporting standards (PMC6693923). (c) Currey and Torous 2023 on missingness in 1,178 mindLAMP participants was excluded as single-platform; it is already catalogued in Module 2's mindLAMP library, so no action | Hassan (the author, not the repo owner) or the journal for (a); a decision for (b) | (b) is the first test of where Module 4's validation-framework scope ends and Module 1's validation evidence begins |
