...
| # | Question | Who / how | Why it matters |
|---|---|---|---|
| 84 | **No platform in Module 2 has documented HIPAA, GDPR/DPA, SOC 2, or 21 CFR Part 11 compliance evidence**, including platforms with explicit clinical-trial or clinical-care positioning (mindLAMP, Avicenna Research, MetricWire) | Direct contact with each organization; see each profile's Key Links | This is the single largest cross-platform gap in the module and the first thing to close before designing any regulated study around any of these platforms |
| 85 | What are actual **Beiwe Service Center (BSC)** rate figures? The fixed-fee-by-duration + variable-fee-by-Active-Participant-Months **methodology** is published, but no example rates are | Beiwe Service Center — https://beiwe.hsph.harvard.edu/ | Needed for any real study budget; the only platform in the module with a *published methodology* but no published *rates* |
| 86 | Does **RADAR-base** offer a managed-hosting or paid-support option comparable to the Beiwe Service Center? | RADAR-base maintainers (King's College London / The Hyve) — https://radar-base.org | RADAR-base's Kafka-based self-hosting requirement may be prohibitive for teams without dedicated DevOps capacity, and no managed alternative was found |
...
| 93 | What is **m-Path**'s pricing model, and what GDPR/compliance documentation exists given its apparent Belgium/KU Leuven origin? | m-Path — https://m-path.io/ | No pricing information was located in this pass |
| 94 | What is **CARP Mobile Sensing**'s realistic adoption track record outside DTU — how many independent research groups have built production studies on the `carp_mobile_sensing` Flutter package? | CARP / Jakob Bardram's group, DTU — https://carp.dk/ | Determines whether CARP is a mature, externally-adopted framework or primarily an internal DTU tool that happens to be open source |
